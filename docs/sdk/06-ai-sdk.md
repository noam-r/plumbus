...
This document defines the **AI SDK**, the programming interface used to integrate AI functionality into Plumbus applications.

Unlike typical frameworks where AI calls are made directly to provider SDKs, Plumbus treats AI as a managed runtime service. This allows the framework to enforce:
...
The goal is to ensure that AI usage remains predictable, observable, and safe.
...
Prompt definitions act as contracts between the application and the AI runtime.
...
Structured outputs allow the runtime to validate AI responses before returning them to application code.

### Model Configuration

Prompt definitions may include model preferences such as provider, model name, and generation settings.
...
Classification should return structured labels rather than free-form responses.
...
Every AI request should record usage metadata, such as:
...
Applications may define AI budgets, such as:
...
Capabilities should avoid sending highly sensitive data to external models unless explicitly allowed.
...
AI should remain predictable infrastructure, not unpredictable application behavior.

## Summary

The AI SDK defines how Plumbus applications integrate with AI systems.

By combining prompt contracts, managed runtime operations, structured outputs, and governance visibility, the framework ensures that AI usage remains:
